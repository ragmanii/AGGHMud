...
void WorldSession::HandleMovementOpcodes(WorldPacket & recvData)
{
   ...
   sLog->outCommand(LOG_FILTER_CHEAT, "AC2-%s > time: %d fall-time: %d | xyzo: %f, %f, %fo(%f) flags[%X] opcode[%s] | transport (xyzo): %f, %f, %fo(%f)",
   ...
   TC_LOG_WARN(LOG_FILTER_CHEAT, "AC2-%s > time: %d fall-time: %d | xyzo: %f, %f, %fo(%f) flags[%X] opcode[%s] | transport (xyzo): %f, %f, %fo(%f)",
       plrMover->GetName(), movementInfo.time, movementInfo.fallTime, movementInfo.pos.m_positionX, movementInfo.pos.m_positionY, movementInfo.pos.m_positionZ, movementInfo.pos.m_orientation,
	movementInfo.flags, LookupOpcodeName(opcode), movementInfo.transport.pos.m_positionX, movementInfo.transport.pos.m_positionY, movementInfo.transport.pos.m_positionZ, movementInfo.transport.pos.m_orientation);
    sLog->outCommand(LOG_FILTER_CHEAT, "AC2-%s Transport > GUID: (low)%d - (high)%d",
        plrMover->GetName(), GUID_LOPART(movementInfo.transport.guid), GUID_HIPART(movementInfo.transport.guid));
    TC_LOG_WARN(LOG_FILTER_CHEAT, "AC2-%s Transport > GUID: (low)%d - (high)%d",
        plrMover->GetName(), GUID_LOPART(movementInfo.transport.guid), GUID_HIPART(movementInfo.transport.guid));
    #endif

    if (plrMover)
    {
        if (World::GetEnableMvAnticheat() && !plrMover->GetCharmerOrOwnerPlayerOrPlayerItself()->IsGameMaster() && !plrMover->GetCharmerOrOwnerPlayerOrPlayerItself()->GetVehicle())
        {
            // calc time deltas
            ...
            #ifdef ANTICHEAT_DEBUG
            sLog->outCommand(LOG_FILTER_CHEAT, "AC2-%s Time > cClientTimeDelta: %d, cServerTime: %d | deltaC: %d - deltaS: %d | SyncTime: %d", plrMover->GetName(), cClientTimeDelta, cServerTime, plrMover->m_anti_DeltaClientTime, plrMover->m_anti_DeltaServerTime, sync_time);
            TC_LOG_WARN(LOG_FILTER_CHEAT, "AC2-%s Time > cClientTimeDelta: %d, cServerTime: %d | deltaC: %d - deltaS: %d | SyncTime: %d", plrMover->GetName(), cClientTimeDelta, cServerTime, plrMover->m_anti_DeltaClientTime, plrMover->m_anti_DeltaServerTime, sync_time);
            #endif
            ...
            if (sync_time > GetMistimingDelta)
            {
                ...
                if (bMistimingModulo)
                {
                    ...
                    sLog->outError(LOG_FILTER_PLAYER, "AC2-%s, mistiming exception #%d, mistiming: %dms", plrMover->GetName(), plrMover->m_anti_MistimingCount, sync_time);
                    TC_LOG_WARN(LOG_FILTER_CHEAT, "AC2-%s, mistiming exception #%d, mistiming: %dms Action: %s", plrMover->GetName(),  plrMover->m_anti_MistimingCount, sync_time, &Warden::Penalty);
                    #endif
                    check_passed = false;
                }
                //if (vehMover)
            ...
            if (!curDest)
            {
                ...
                {
                    #ifdef ANTICHEAT_EXCEPTION_INFO
                    sLog->outError(LOG_FILTER_PLAYER, "AC2-%s, AntiGravity exception. JumpHeight = %f, Allowed Vertical Speed = %f",
                        plrMover->GetName(), JumpHeight, plrMover->m_anti_Last_VSpeed);
                    TC_LOG_WARN(LOG_FILTER_CHEAT, "AC2-%s, AntiGravity exception. JumpHeight = %f, Allowed Vertical Speed = %f",
                        plrMover->GetName(), JumpHeight, plrMover->m_anti_Last_VSpeed);
                    #endif
                    check_passed = false;
                    //if (vehMover)
                ...
                if (real_delta > allowed_delta)
                {
                    ...
                    {
                        sLog->outError(LOG_FILTER_PLAYER, "AC2-%s, speed exception | cDelta=%f aDelta=%f | cSpeed=%f lSpeed=%f deltaTime=%f", plrMover->GetName(), real_delta, allowed_delta, current_speed, plrMover->m_anti_Last_HSpeed, time_delta);
                        TC_LOG_WARN(LOG_FILTER_CHEAT, "AC2-%s, speed exception | cDelta=%f aDelta=%f | cSpeed=%f lSpeed=%f deltaTime=%f", plrMover->GetName(), real_delta, allowed_delta, current_speed, plrMover->m_anti_Last_HSpeed, time_delta);
                    } else {
                        sLog->outError(LOG_FILTER_PLAYER, "AC2-%s, teleport exception | cDelta=%f aDelta=%f | cSpeed=%f lSpeed=%f deltaTime=%f", plrMover->GetName(), real_delta, allowed_delta, current_speed, plrMover->m_anti_Last_HSpeed, time_delta);
                        TC_LOG_WARN(LOG_FILTER_CHEAT, "AC2-%s, teleport exception | cDelta=%f aDelta=%f | cSpeed=%f lSpeed=%f deltaTime=%f", plrMover->GetName(), real_delta, allowed_delta, current_speed, plrMover->m_anti_Last_HSpeed, time_delta);
                    }
                    #endif
                    check_passed = false;
                    //if (vehMover)
                ...
                {
                    #ifdef ANTICHEAT_EXCEPTION_INFO
                    sLog->outError(LOG_FILTER_PLAYER, "AC2-%s, mountain exception | tg_z=%f", plrMover->GetName(), tg_z);
                    TC_LOG_WARN(LOG_FILTER_CHEAT, "AC2-%s, mountain exception | tg_z=%f", plrMover->GetName(), tg_z);
                    #endif
                    check_passed = false;
                //    if (vehMover)
                //        vehMover->Die();
                ...
                if (no_fly_auras && !no_fly_flags)
                {
                    ...
                    sLog->outError(LOG_FILTER_PLAYER, "AC2-%s, flight exception. {SPELL_AURA_FLY=[%X]} {SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED=[%X]} {SPELL_AURA_MOD_INCREASE_FLIGHT_SPEED=[%X]} {SPELL_AURA_MOD_MOUNTED_FLIGHT_SPEED_ALWAYS=[%X]} {SPELL_AURA_MOD_FLIGHT_SPEED_NOT_STACK=[%X]} {plrMover->GetVehicle()=[%X]}",
                        plrMover->GetName(),
                    ...
                    TC_LOG_WARN(LOG_FILTER_CHEAT, "AC2-%s, flight exception. {SPELL_AURA_FLY=[%X]} {SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED=[%X]} {SPELL_AURA_MOD_INCREASE_FLIGHT_SPEED=[%X]} {SPELL_AURA_MOD_MOUNTED_FLIGHT_SPEED_ALWAYS=[%X]} {SPELL_AURA_MOD_FLIGHT_SPEED_NOT_STACK=[%X]} {plrMover->GetVehicle()=[%X]}",
                        plrMover->GetName(),
                        plrMover->HasAuraType(SPELL_AURA_FLY), plrMover->HasAuraType(SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED),
                        plrMover->HasAuraType(SPELL_AURA_MOD_INCREASE_FLIGHT_SPEED), plrMover->HasAuraType(SPELL_AURA_MOD_MOUNTED_FLIGHT_SPEED_ALWAYS),
                        plrMover->HasAuraType(SPELL_AURA_MOD_FLIGHT_SPEED_NOT_STACK), plrMover->GetVehicle());
                ...
                {
                    #ifdef ANTICHEAT_EXCEPTION_INFO
                    sLog->outError(LOG_FILTER_PLAYER, "AC2-%s, waterwalk exception. [%X]{SPELL_AURA_WATER_WALK=[%X]}",
                        plrMover->GetName(), movementInfo.flags, plrMover->HasAuraType(SPELL_AURA_WATER_WALK));
                    TC_LOG_WARN(LOG_FILTER_CHEAT, "AC2-%s, waterwalk exception. [%X]{SPELL_AURA_WATER_WALK=[%X]}",
                        plrMover->GetName(), movementInfo.flags, plrMover->HasAuraType(SPELL_AURA_WATER_WALK));
                    #endif
                    check_passed = false;
                    //if (vehMover)
                ...
                if (no_swim_in_water && movementInfo.pos.GetPositionZ() < 0.0001f && movementInfo.pos.GetPositionZ() > -0.0001f)
                {
                    if (const Map *map = plrMover->GetMap())
                    {
                        ...
                        {
                            #ifdef ANTICHEAT_DEBUG
                            sLog->outDebug(LOG_FILTER_PLAYER, "AC2-%s, teleport to plane exception. plane_z: %f", plrMover->GetName(), plane_z);
                            TC_LOG_WARN(LOG_FILTER_CHEAT, "AC2-%s, teleport to plane exception. plane_z: %f", plrMover->GetName(), plane_z);
                            #endif
                            #ifdef ANTICHEAT_EXCEPTION_INFO
                            if (plrMover->m_anti_TeleToPlane_Count > World::GetTeleportToPlaneAlarms())
                            {
                                sLog->outError(LOG_FILTER_PLAYER, "AC2-%s, teleport to plane exception. Exception count: %d", plrMover->GetName(), plrMover->m_anti_TeleToPlane_Count);
 						        TC_LOG_WARN(LOG_FILTER_CHEAT, "AC2-%s, teleport to plane exception. Exception count: %d", plrMover->GetName(), plrMover->m_anti_TeleToPlane_Count);
                                /* Disabled, not passive at all, and apparently causing crashes:
                                sWorld.SendWorldText(3, strcat("Kicking cheater: ", plrMover->GetName()));
                                KickPlayer();
                                return; */
    ...
    if (plrMover && !vehMover)                               // nothing is charmed, or player charmed
    {
        ...
        {
            sLog->outError(LOG_FILTER_PLAYER, "AC2-%s produce %d anticheat alarms", LogMessage::getTimeStr(time(NULL)), plrMover->GetName(), plrMover->m_anti_AlarmCount);
            TC_LOG_WARN(LOG_FILTER_CHEAT, "AC2-%s produce %d anticheat alarms.", plrMover->GetName(), plrMover->m_anti_AlarmCount);
...
void WorldSession::HandleMoveKnockBackAck(WorldPacket & recvData)
{
    ...
    #ifdef ANTICHEAT_DEBUG
	sLog->outCommand(LOG_FILTER_CHEAT, "%s CMSG_MOVE_KNOCK_BACK_ACK: time: %d, fall time: %d | xyzo: %f,%f,%fo(%f) flags[%X] Vspeed: %f, Hspeed: %f", GetPlayer()->GetName(), movementInfo.time, movementInfo.fallTime, movementInfo.pos.m_positionX, movementInfo.pos.m_positionY, movementInfo.pos.m_positionZ, movementInfo.pos.m_orientation, movementInfo.flags, movementInfo.jump.zspeed, movementInfo.jump.xyspeed);
    TC_LOG_WARN(LOG_FILTER_CHEAT, "%s CMSG_MOVE_KNOCK_BACK_ACK: time: %d, fall time: %d | xyzo: %f,%f,%fo(%f) flags[%X] Vspeed: %f, Hspeed: %f", GetPlayer()->GetName(), movementInfo.time, movementInfo.fallTime, movementInfo.pos.m_positionX, movementInfo.pos.m_positionY, movementInfo.pos.m_positionZ, movementInfo.pos.m_orientation, movementInfo.flags, movementInfo.jump.zspeed, movementInfo.jump.xyspeed);
    #endif
