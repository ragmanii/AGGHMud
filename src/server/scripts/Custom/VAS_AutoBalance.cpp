...
class VAS_AutoBalance_WorldScript : public WorldScript
{
    ...
    void SetInitialWorldSettings()
    {
        ...
        TC_LOG_DEBUG(LOG_FILTER_WORLDSERVER, "  Powered by {VAS} AutoBalance");
        TC_LOG_DEBUG(LOG_FILTER_WORLDSERVER, "----------------------------------------------------");
        TC_LOG_DEBUG(LOG_FILTER_WORLDSERVER, "  xPlayer = %4.1f ", sWorld->getFloatConfig(VAS_Config_xPlayer));
        TC_LOG_DEBUG(LOG_FILTER_WORLDSERVER, "  AutoInstance = %u ", sWorld->getIntConfig(VAS_AutoInstance));
        TC_LOG_DEBUG(LOG_FILTER_WORLDSERVER, "  PlayerChangeNotify = %u ", sWorld->getIntConfig(VAS_PlayerChangeNotify));
        TC_LOG_DEBUG(LOG_FILTER_WORLDSERVER, "  Min.D.Mod = %4.2f ", sWorld->getFloatConfig(VAS_Min_D_Mod));
        TC_LOG_DEBUG(LOG_FILTER_WORLDSERVER, "  Min.HP.Mod = %4.2f ", sWorld->getFloatConfig(VAS_Min_HP_Mod));
        TC_LOG_DEBUG(LOG_FILTER_WORLDSERVER, "  VasDebug   =  %u ", GetValidDebugLevel());
        TC_LOG_DEBUG(LOG_FILTER_WORLDSERVER, "----------------------------------------------------\n");
    }
...
class VAS_AutoBalance_PlayerScript : public PlayerScript
{
    ...
    void OnLogin(Player *Player)
    {
        ...
        if (GetValidDebugLevel() >= 3)
            TC_LOG_DEBUG(LOG_FILTER_PLAYER_LOADING, "### VAS_AutoBalance_PlayerScript - OnLogin Player=%s", Player->GetName());
    }
};
...
class VAS_AutoBalance_UnitScript : public UnitScript
{
    ...
    uint32 DealDamage(Unit* AttackerUnit, Unit *playerVictim, uint32 damage, DamageEffectType damagetype)
    {
        if (AttackerUnit->GetMap()->IsDungeon() && playerVictim->GetMap()->IsDungeon())
            ...
            {
                if (GetValidDebugLevel() >= 3)
                    TC_LOG_DEBUG(LOG_FILTER_TSCR, "### VAS_AutoBalance_UnitScript - VAS_Unit_DealDamage Attacker=%s Victim=%s Start Damage=%u",AttackerUnit->GetName(),playerVictim->GetName(),damage);
                damage = VAS_Modifer_DealDamage(AttackerUnit,damage);
                if (GetValidDebugLevel() >= 3)
                    TC_LOG_DEBUG(LOG_FILTER_TSCR, "### VAS_AutoBalance_UnitScript - VAS_Unit_DealDamage Attacker=%s Victim=%s End Damage=%u",AttackerUnit->GetName(),playerVictim->GetName(),damage);
            }
            return damage;
    }
    ...
    void ModifyPeriodicDamageAurasTick(Unit* target, Unit* attacker, uint32& damage)
    {
        if (attacker->GetMap()->IsDungeon() && target->GetMap()->IsDungeon())
		{
            ...
            {
                if (GetValidDebugLevel() >= 3)
                    TC_LOG_DEBUG(LOG_FILTER_TSCR, "### VAS_AutoBalance_UnitScript - VAS_Unit_HandlePeriodicDamage Attacker=%s Victim=%s Start Damage=%u",attacker->GetName(),target->GetName(),damage);
                ...
                if (GetValidDebugLevel() >= 3)
                    TC_LOG_DEBUG(LOG_FILTER_TSCR, "### VAS_AutoBalance_UnitScript - VAS_Unit_HandlePeriodicDamage Attacker=%s Victim=%s End Damage=%u",attacker->GetName(),target->GetName(),damage);
            }
		}
    }
    ...
    void ModifySpellDamageTaken(Unit* target, Unit* attacker, int32& damage)
    {
        if ((attacker->GetMap()->IsDungeon() && target->GetMap()->IsDungeon()) || ( attacker->GetMap()->IsBattleground() && target->GetMap()->IsBattleground()))
        {
            ...
            {
                if (GetValidDebugLevel() >= 3)
                    TC_LOG_DEBUG(LOG_FILTER_TSCR, "### VAS_AutoBalance_UnitScript - CalculateSpellDamageTaken Attacker=%s Victim=%s Start Damage=%u",attacker->GetName(),target->GetName(),damage);
                ...
                if (GetValidDebugLevel() >= 3)
                    TC_LOG_DEBUG(LOG_FILTER_TSCR, "### VAS_AutoBalance_UnitScript - CalculateSpellDamageTaken Attacker=%s Victim=%s End Damage=%u",attacker->GetName(),target->GetName(),damage);
            }
        }
            return;
    }
	...
	void ModifyHealRecieved(Unit* target, Unit* victim, uint32& addHeal)
    {
        if (victim->GetMap()->IsDungeon() && victim->GetMap()->IsDungeon())
        {
            ...
            {
                if (GetValidDebugLevel() >= 3)
                    TC_LOG_DEBUG(LOG_FILTER_TSCR, "### VAS_AutoBalance_UnitScript - ModifyHealRecieved Attacker=%s Victim=%s Start Damage=%u",victim->GetName(),target->GetName(),addHeal);

                float multiplyer = (float)CreatureInfo[victim->GetGUID()].DamageMultiplier;
				addHeal = addHeal * multiplyer;

                if (GetValidDebugLevel() >= 3)
                    TC_LOG_DEBUG(LOG_FILTER_TSCR, "### VAS_AutoBalance_UnitScript - ModifyHealRecieved Attacker=%s Victim=%s End Damage=%u",victim->GetName(),target->GetName(),addHeal);
            }
        }
            return;
    }
    ...
    void ModifyMeleeDamage(Unit* target, Unit* attacker, uint32& damage)
    {
        ...
        if (((attacker->GetMap()->IsDungeon() && target->GetMap()->IsDungeon()) || (attacker->GetMap()->IsBattleground() && target->GetMap()->IsBattleground())) && (attacker->GetTypeId() != TYPEID_PLAYER))
            ...
            {
                if (GetValidDebugLevel() >= 3)
                    TC_LOG_DEBUG(LOG_FILTER_TSCR, "### VAS_AutoBalance_UnitScript - CalculateMeleeDamage Attacker=%s Victim=%s Start Damage=%u",attacker->GetName(),target->GetName(),damage);
                ...
                if (GetValidDebugLevel() >= 3)
                    TC_LOG_DEBUG(LOG_FILTER_TSCR, "### VAS_AutoBalance_UnitScript - CalculateMeleeDamage Attacker=%s Victim=%s End Damage=%u",attacker->GetName(),target->GetName(),damage);
            }
            return;
    }
...
class VAS_AutoBalance_AllMapScript : public AllMapScript
{
    ...
    void OnPlayerEnterAll(Map* map, Player* player)
    {
        if (GetValidDebugLevel() >= 2)
        {
            ...
            TC_LOG_DEBUG(LOG_FILTER_TSCR, "## VAS_AutoBalance_AllMapScript - OnPlayerEnterAll");
            TC_LOG_DEBUG(LOG_FILTER_TSCR, "## For InsatanceID %u",map->GetInstanceId());
            TC_LOG_DEBUG(LOG_FILTER_TSCR, "## IsDungeon= %u",map->GetEntry()->IsDungeon());
            TC_LOG_DEBUG(LOG_FILTER_TSCR, "## For Map %u",player->GetMapId());
            TC_LOG_DEBUG(LOG_FILTER_TSCR, "## PlayersInMap %u",map->GetPlayersCountExceptGMs());
            TC_LOG_DEBUG(LOG_FILTER_TSCR, "## pDifficulty %u",uint32(player->GetDifficulty(player->GetMap()->IsHeroic())));
            TC_LOG_DEBUG(LOG_FILTER_TSCR, "## pGetDungeonDifficulty %u",uint32(player->GetDungeonDifficulty()));
            TC_LOG_DEBUG(LOG_FILTER_TSCR, "## pGetRaidDifficulty %u",uint32(player->GetRaidDifficulty()));
            TC_LOG_DEBUG(LOG_FILTER_TSCR, "## maxPlayers %u",((InstanceMap*)sMapMgr->FindMap(player->GetMapId(), player->GetInstanceId()))->GetMaxPlayers());
    ...
    void OnPlayerLeaveAll(Map* map, Player* player)
    {
        ...
        if (GetValidDebugLevel() >= 3)
            TC_LOG_DEBUG(LOG_FILTER_TSCR, "#### VAS_AutoBalance_AllMapScript - OnPlayerLeaveAll map=%s player=%s", map->GetMapName(),player->GetName());
        ...
        if (instancePlayerCount >=1)
        {
            ...
            {
                TC_LOG_DEBUG(LOG_FILTER_TSCR, "----------------------------------------------------");
                TC_LOG_DEBUG(LOG_FILTER_TSCR, "## VAS_AutoBalance_AllMapScript - OnPlayerLeaveAll");
                TC_LOG_DEBUG(LOG_FILTER_TSCR, "## For InsatanceID %u",map->GetInstanceId());
                TC_LOG_DEBUG(LOG_FILTER_TSCR, "## IsDungeon= %u",map->GetEntry()->IsDungeon());
                TC_LOG_DEBUG(LOG_FILTER_TSCR, "## For Map %u",player->GetMapId());
                TC_LOG_DEBUG(LOG_FILTER_TSCR, "## PlayersInMap %u",instancePlayerCount);
                TC_LOG_DEBUG(LOG_FILTER_TSCR, "----------------------------------------------------\n");
            }
...
class VAS_AutoBalance_WorldMapScript : public WorldMapScript
{
    ...
    void OnPlayerEnter(Map* map, Player* player)
    {
        ...
        if (GetValidDebugLevel() >= 3)
            TC_LOG_DEBUG(LOG_FILTER_TSCR, "### VAS_AutoBalance_WorldMapScript - OnPlayerEnter Map=%s player=%s",map->GetMapName(),player->GetName());
    }
    ...
    void OnPlayerLeave(Map* map, Player* player)
    {
        ...
        if (GetValidDebugLevel() >= 3)
            TC_LOG_DEBUG(LOG_FILTER_TSCR, "### VAS_AutoBalance_WorldMapScript - OnPlayerLeave Map=%s player=%s",map->GetMapName(),player->GetName());
    }
};
...
class VAS_AutoBalance_AllCreatureScript : public AllCreatureScript
{
    ...
    void ModifyCreatureAttributes(Creature* creature)
    {
        ...
        if((GetValidDebugLevel() >= 3))
        {
            TC_LOG_DEBUG(LOG_FILTER_TSCR, "## VAS-AutoBalance MobID=%u MapID=%u creatureName=%s  GUID=%llu  instancePlayerCount=%u", creatureTemplate->Entry, creature->GetMapId(), creatureTemplate->Name.c_str(), creature->GetGUID(), instancePlayerCount);
            TC_LOG_DEBUG(LOG_FILTER_TSCR, "## VAS-AutoBalance MapDifficulty=%u Health=%u / %u healthMultiplier=%4.5f", creature->GetMap()->GetDifficulty(), scaledHealth, baseHealth, healthMultiplier);
            TC_LOG_DEBUG(LOG_FILTER_TSCR, "## VAS-AutoBalance maxNumberOfPlayers=%u IsRaid=%s", maxNumberOfPlayers, BOOL_TO_STRING(creature->GetMap()->IsRaid()));
            TC_LOG_DEBUG(LOG_FILTER_TSCR, "## VAS-AutoBalance Mana %u / %u", baseMana, scaledMana);
            TC_LOG_DEBUG(LOG_FILTER_TSCR, "## VAS-AutoBalance damageMultiplier=%4.2f", damageMultiplier);
        }
